...
test.describe('D&D 기능 테스트', () => {
  ...
  test('유저는 월간뷰에서 캘린더의 셀을 드래그해서 일정을 수정하고 완료 스낵바가 뜬다', async ({ page }) => {
    // Given: 애플리케이션이 로드됨
  ...
  test("월간뷰에서 일정이 겹치면 다이얼로그 출력 '예' 버튼을 클릭하면 일정이 수정된다", async ({ page }) => {
    // Given: 애플리케이션이 로드됨
    await expect(page.getByRole('heading', { name: '일정 추가' })).toBeVisible();
  ...
  test('유저는 주간뷰에서 일정을 드래그해서 다른 날짜로 이동시키면 일정이 수정되고 완료 스낵바가 뜬다', async ({ page }) => {
    // Given: 애플리케이션이 로드됨
    await expect(page.getByRole('heading', { name: '일정 추가' })).toBeVisible();
  ...
  test("주간뷰에서 일정이 겹치면 다이얼로그 출력 '예' 버튼을 클릭하면 일정이 수정된다", async ({ page }) => {
    // Given: 애플리케이션이 로드됨
    await expect(page.getByRole('heading', { name: '일정 추가' })).toBeVisible();
  ...
  test("반복 일정을 다른 셀로 드래그시 단독 일정으로 수정" , async ({ page }) => {
    ...
    await page.getByRole('cell', { name: '22 반복일정' }).dragTo(page.getByRole('cell', { name: '18' }));
    ...
    const repeatIcon = eventBox.locator('svg').filter({ hasText: '' });
